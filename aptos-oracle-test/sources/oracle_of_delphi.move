...
module overmind::price_oracle {
    ...
    const MAXIMUM_FRESH_DURATION_SECONDS: u64 = 10800;
    ...
    */
    fun init_module(admin: &signer) {
        ...
        let price_feed_updated_event = account::new_event_handle<PriceFeedUpdatedEvent>(&resource);

        let price_board = PriceBoard { prices: table::new() };
        let state = State { signer_cap: resource_signer_cap, price_feed_updated_event: price_feed_updated_event };

        move_to(&resource, price_board);
    ...
    */
    public entry fun update_price_feed(
        ...
        assert!(signer::address_of(admin) == account::get_signer_capability_address(&state.signer_cap), ErrorCodeForAllAborts);
        ...
        // Access the PriceBoard
        let price_board = borrow_global_mut<PriceBoard>(signer::address_of(admin));

        // Update the price feed for the specified pair
    ...
    */
    public fun get_price(pair: String): Price acquires PriceBoard {
        ...
        // Check if the pair exists in the PriceBoard
        ...
        assert!(
            now <= price_feed.latest_attestation_timestamp_seconds + MAXIMUM_FRESH_DURATION_SECONDS,
            ErrorCodeForAllAborts
        );
    ...
    */
    public fun get_price_no_older_than(pair: String, maximum_age_seconds: u64): Price acquires PriceBoard {
        let price_board = borrow_global_mut<PriceBoard>(@overmind);
        let price_feed = get_price_feed(price_board, pair);
        let now = timestamp::now_seconds();

        assert!(!is_price_stale(price_feed, now, maximum_age_seconds), ErrorCodeForAllAborts);
    ...
    */
    ...
    // Helper functions
    //==============================================================================================
    fun get_price_feed(price_board: &PriceBoard, pair: String): &PriceFeed {
        assert!(table::contains(&price_board.prices, pair), ErrorCodeForAllAborts);
        table::borrow(&price_board.prices, pair)
    }

    //==============================================================================================
    // Validation functions
    //==============================================================================================
    fun is_price_stale(price_feed: &PriceFeed, now: u64, max_duration: u64): bool {
        now > price_feed.latest_attestation_timestamp_seconds + max_duration
    }
    //==============================================================================================
    // Tests - DO NOT MODIFY
    //==============================================================================================
